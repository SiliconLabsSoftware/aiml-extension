...
sl_status_t slx_ml_ov2640_init(const arducam_config_t *config)
{
    ...
    SL_VERIFY(slx_ml_arducam_driver_i2c_write_regs(ov2640_default_regs, NULL, 0));
    SL_VERIFY(sli_set_format(config->data_format));
    SL_VERIFY(sli_set_framesize(config->sensor_resolution, config->image_resolution.width, config->image_resolution.height));
...
sl_status_t slx_ml_ov2640_set_setting(arducam_setting_t setting, int32_t value)
{
    ...
    case ARDUCAM_SETTING_BRIGHTNESS:
        result = sli_set_brightness(value);
        break;
    case ARDUCAM_SETTING_CONTRAST:
        result =  sli_set_contrast(value);
        break;
    case ARDUCAM_SETTING_SATURATION:
        result = sli_set_saturation(value);
        break;
    case ARDUCAM_SETTING_GAINCEILING:
        result = sli_set_gainceiling(value);
        break;
    case ARDUCAM_SETTING_MIRROR:
        result = sli_set_mirror(value);
        break;
    case ARDUCAM_SETTING_FLIP:
        result = sli_set_flip(value);
        break;
    case ARDUCAM_SETTING_SPECIALEFFECT:
        result = sli_set_specialeffect(value);
        break;
    default:
...
static sl_status_t sli_check_comms_interface(void)
{
    ...
    slx_ml_arducam_driver_i2c_read_reg(MIDH, &id.Manufacturer_ID1);
    slx_ml_arducam_driver_i2c_read_reg(MIDL, &id.Manufacturer_ID2 );
    slx_ml_arducam_driver_i2c_read_reg(REG_PID, &id.PIDH);
    slx_ml_arducam_driver_i2c_read_reg(REG_VER, &id.PIDL);
...
static sl_status_t sli_set_format(int format)
{
    ...
    SL_VERIFY(slx_ml_arducam_driver_i2c_write_regs(regs, NULL, 0));
...
static sl_status_t sli_set_framesize(arducam_resolution_t resolution, uint32_t width, uint32_t height)
{
    ...
    SL_VERIFY(slx_ml_arducam_driver_i2c_write_reg(ZMOW, ZMOW_OUTW_SET(width)));
    SL_VERIFY(slx_ml_arducam_driver_i2c_write_reg(ZMOH, ZMOH_OUTH_SET(height)));
    SL_VERIFY(slx_ml_arducam_driver_i2c_write_reg(ZMHH, ZMHH_OUTW_SET(width) | ZMHH_OUTH_SET(height)));
    SL_VERIFY(slx_ml_arducam_driver_i2c_write_reg(RESET, 0x00));
...
static sl_status_t sli_set_contrast(int level)
{
    ...
    for (uint32_t i=0; i< ARRAY_COUNT(ov2640_contrast_regs[0]); i++)
    {
...
static sl_status_t sli_set_saturation(int level)
{
    ...
    /* Write contrast registers */
    for (uint32_t i=0; i< ARRAY_COUNT(ov2640_saturation_regs[0]); i++)
    {
...
static sl_status_t sli_set_brightness(int level)
{
    ...
    /* Write contrast registers */
    for (uint32_t i=0; i< ARRAY_COUNT(ov2640_brightness_regs[0]); i++)
    {
...
static sl_status_t sli_set_flip(int flip)
{
    uint8_t reg04;
    OV2640_WRITE_VERIFY(BANK_SEL, BANK_SEL_SENSOR);
    SL_VERIFY(slx_ml_arducam_driver_i2c_read_reg( 0x04, &reg04));
    if( flip == 0 )
    {
        reg04 &= 0xAF;
    }
...
static sl_status_t sli_set_specialeffect(int effect)
{
    ...
    /* Write contrast registers */
    for (uint32_t i=0; i< ARRAY_COUNT(ov2640_special_effect_regs[0]); i++)
    {
